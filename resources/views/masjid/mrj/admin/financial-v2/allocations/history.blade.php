        ...
        <section class="mt-6 space-y-3">
            @forelse ($allocationHistory as $item)
                ...
                <article class="rounded-2xl bg-base-100 p-4 shadow-sm ring-1 ring-base-300 sm:p-5">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p class="font-bold">{{ $version?->fundings?->pluck('fund.name')->filter()->join(' + ') ?: ($allocation->fund?->name ?? 'Dana') }}{{ $allocation->program ? ' · '.$allocation->program->name : '' }}</p><p class="mt-1 text-sm text-base-content/70">{{ $allocation->reason }}</p><p class="mt-2 text-xs text-base-content/55">{{ $allocation->allocation_reference }} · Berlaku {{ $version?->effective_from?->translatedFormat('d M Y') ?? '—' }} · {{ $statuses[$allocation->status] ?? ucfirst($allocation->status) }}</p></div><div class="text-sm sm:text-right"><p>Total <strong>{{ $rupiah($item['allocated']) }}</strong></p><p>Realisasi {{ $rupiah($item['realized']) }}</p><p>Sisa <strong>{{ $rupiah($item['remaining']) }}</strong></p></div></div>
                    @if($version?->fundings?->isNotEmpty())<div class="mt-3 flex flex-wrap gap-2">@foreach($version->fundings as $funding)<span class="badge badge-ghost h-auto py-1">{{ $funding->fund?->name ?? 'Dana' }} · {{ $rupiah($funding->amount) }}</span>@endforeach</div>@endif
                    ...
                    @if ($item['realizations']->isNotEmpty())<div class="mt-4 border-t border-base-200 pt-3"><p class="text-xs font-semibold text-base-content/60">Pembayaran realisasi tercatat</p><ul class="mt-2 space-y-2 text-sm text-base-content/70">@foreach($item['realizations'] as $realization)<li>{{ $realization->transaction?->accounting_date?->translatedFormat('d M Y') }} · {{ $realization->transaction?->source_reference }} · {{ $rupiah($realization->transaction?->gross_amount) }}{{ $realization->transaction?->primaryFinancialAccount ? ' · '.$realization->transaction->primaryFinancialAccount->name : '' }}<span class="block text-xs text-base-content/55">{{ $realization->transaction?->splits?->map(fn($split) => ($split->fund?->name ?? 'Dana').' '.$rupiah($split->split_amount))->join(' + ') }}</span></li>@endforeach</ul></div>@endif
                </article>
            @empty
