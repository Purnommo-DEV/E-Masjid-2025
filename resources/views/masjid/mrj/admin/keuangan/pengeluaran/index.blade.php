...
@section('title', 'Pengeluaran Umum')
...
<style>
    /* Reuse style seperti Petty Cash */
    ...
    <div class="card-header" role="banner">
        <div>
            <h3 class="title flex items-center gap-2">
                ...
                <span class="text-sm font-normal opacity-90">(Gaji, Listrik, Honor, dll)</span>
            </h3>
            <p class="subtitle">
                Pengeluaran rutin & nilai besar. Untuk pengeluaran kecil gunakan menu <strong>Petty Cash</strong>.
            </p>
        </div>
        ...
        <button type="button"
                ...
                aria-controls="pengeluaranModal">
            ...
            <span class="text-sm font-semibold">Transaksi Baru</span>
        </button>
    </div>

    <div class="card-body">
        <div class="alert alert-info shadow-sm mb-4 text-sm flex items-start gap-2">
            <span class="mt-0.5">ℹ️</span>
            <div>
                <strong>Info:</strong> Menu ini hanya untuk pengeluaran besar/rutin
                (<span class="font-medium">gaji, listrik, internet, honor, dll</span>).<br>
                Pengeluaran kecil (ATK, sabun, air mineral) → gunakan menu
...
<!-- Modal Pengeluaran pakai <dialog> seperti Petty Cash -->
<dialog id="pengeluaranModal" class="modal" aria-labelledby="pengeluaranModalTitle" role="dialog">
  <div class="modal-panel w-11/12 max-w-3xl">
    <form id="formPengeluaran"
        ...
        <div class="flex items-start justify-between gap-4 mb-2">
            <div>
                <h3 id="pengeluaranModalTitle" class="text-xl font-extrabold text-emerald-800">
                    🧾 Input Pengeluaran Umum
                </h3>
                <p class="text-sm text-slate-500 mt-1">
                    Catat pengeluaran besar/rutin seperti gaji, listrik, honor, dan lainnya.
                </p>
            </div>
            <div>
                <button type="button"
                        id="closePengeluaranModalBtn"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
            <div class="space-y-2 lg:col-span-1">
                <label class="text-sm font-medium text-emerald-700">
                    Akun Beban <span class="text-red-500">*</span>
                </label>
                <select name="akun_beban_id"
                        class="w-full rounded-lg border border-emerald-100 px-3 py-2 shadow-sm select2"
                        required>
                    <option value="">— Pilih Akun Beban Besar —</option>
                    @foreach(\App\Models\AkunKeuangan::where('tipe','beban')
                            ->where('jenis_beban','besar')
                            ->orderBy('kode')
                            ->get() as $akun)
                ...
                <small class="text-xs text-slate-500">
                    Hanya akun dengan jenis <span class="font-semibold">Besar</span> yang muncul di sini.
                </small>
            </div>
        ...
        <div class="mt-4">
            ...
            <textarea name="keterangan"
                      ...
                      rows="3"
                      required
                      placeholder="Contoh: Gaji Imam bulan November 2025"></textarea>
        </div>

        <div class="flex items-center justify-end gap-3 mt-5">
            <button type="button"
                    id="cancelPengeluaranBtn"
...
$(document).ready(function() {
    ...
    // Select2 di dalam dialog, dropdownParent ke .modal-panel
    if ($.fn.select2) {
        const $dp = $('#pengeluaranModal .modal-panel').length
            ? $('#pengeluaranModal .modal-panel')
            : $(document.body);

        $('.select2').select2({
            dropdownParent: $dp,
            placeholder: "Pilih akun beban besar",
            allowClear: true,
            width: '100%'
        });
    }

    // FORMAT RIBUAN UNTUK NOMINAL (sama seperti sebelumnya)
    ...
    // Open modal dari tombol header
    window.openPengeluaranModal = function() {
        form[0].reset();
        ...
        setTimeout(() => {
            $('select[name="akun_beban_id"]').trigger('focus');
        }, 150);
    };

    // Tombol close/cancel dialog
    $('#closePengeluaranModalBtn, #cancelPengeluaranBtn').on('click', function() {
        closeDialog(modal);
    });
    if (modal) {
    ...
    // DataTables
    const tablePengeluaran = $('#tabelPengeluaran').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.keuangan.pengeluaran.data") }}',
        columns: [
    ...
    // Submit form (disesuaikan ke dialog, tanpa Bootstrap modal)
    form.on('submit', function(e) {
        e.preventDefault();

        const $btn = $('#submitPengeluaranBtn');
        ...
        $.ajax({
            url: '{{ route("admin.keuangan.pengeluaran.store") }}',
            method: 'POST',
            data: $(this).serialize(),
            beforeSend: () => {
            ...
            success: function(res) {
                Swal.fire('Sukses!', res.message || 'Pengeluaran berhasil dicatat!', 'success');
                closeDialog(modal);
                form[0].reset();
                if ($.fn.select2) {
            ...
            error: function(xhr) {
                const msg = xhr.responseJSON?.message || 'Gagal menyimpan pengeluaran';
                Swal.fire('Error!', msg, 'error');
            },
            complete: () => {
                $btn.prop('disabled', false)
                    .removeClass('opacity-80')
                    .html('<span class="icon-area"></span><span>Simpan Pengeluaran</span>');
            }
        });
    });
});
