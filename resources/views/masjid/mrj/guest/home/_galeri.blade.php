{{-- === GALERI + MODAL === --}}
<section class="home-section home-section-galeri relative overflow-hidden bg-pattern-islamic py-16">
    <div class="container relative mx-auto flex flex-col items-center px-6 text-center lg:px-16 xl:px-24">
        
        {{-- Heading --}}
        <div class="mx-auto mb-8 w-full max-w-4xl text-center sm:mb-10">
        ...
        <div class="mx-auto flex w-full justify-center">
            <div class="mx-auto grid w-full max-w-6xl grid-flow-dense grid-cols-2 place-content-center place-items-center gap-3 auto-rows-[4.6rem] sm:grid-cols-4 sm:gap-4 sm:auto-rows-[5rem] lg:auto-rows-[5.4rem]">
                @forelse($galeri as $g)
                    @php
                        $pattern = $loop->index % 4;

                        $spanClass = match ($pattern) {
                            0 => 'col-span-2 row-span-2 sm:col-span-1 sm:row-span-4',
                            1 => 'col-span-1 row-span-3 sm:col-span-2 sm:row-span-2',
                            2 => 'col-span-1 row-span-3 sm:col-span-2 sm:row-span-2',
                            default => 'col-span-2 row-span-2 sm:col-span-1 sm:row-span-4',
                        };
                    @endphp

                    <button type="button"
                        class="group relative h-full w-full justify-self-center overflow-hidden rounded-2xl border border-white/80 bg-white text-left shadow-xl shadow-emerald-900/5 ring-1 ring-emerald-100/50 transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl hover:shadow-emerald-200/40 sm:rounded-[1.5rem] {{ $spanClass }}"
                ...
                @empty
                    <div class="col-span-full mx-auto w-full max-w-xl rounded-[2rem] border border-dashed border-emerald-200 bg-white/80 px-6 py-12 text-center shadow-xl shadow-emerald-900/5">
        ...
        <dialog id="galeriModal" class="modal">
            <div class="modal-box max-w-4xl overflow-hidden rounded-2xl bg-white p-0">
                <div class="relative bg-slate-900/5">
                    ...
                    <button id="galeriPrev" aria-label="Sebelumnya"
                        class="absolute left-3 top-1/2 z-50 inline-flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 shadow-lg transition hover:scale-105">
                    ...
                    <button id="galeriNext" aria-label="Berikutnya"
                        class="absolute right-3 top-1/2 z-50 inline-flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 shadow-lg transition hover:scale-105">
                        <svg class="h-5 w-5 text-emerald-700" viewBox="0 0 24 24" fill="none">
                    ...
                    <button id="closeGaleriModalBtn" type="button"
                        class="absolute right-3 top-3 z-50 inline-flex h-9 w-9 items-center justify-center rounded-md bg-black/60 text-white shadow-sm">
                        ✕
                    </button>
                </div>
                ...
                <div class="p-4 text-left">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <div id="galeriCounter" class="mt-1 text-xs text-slate-500">0 / 0</div>
                        </div>
                    ...
                    <div class="mt-3 text-xs text-slate-400">
                        Gunakan panah kiri/kanan pada keyboard untuk navigasi. Klik thumbnail untuk berpindah.
                    </div>
                </div>
            </div>
