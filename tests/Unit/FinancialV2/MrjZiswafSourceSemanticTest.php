...
test('source fund history remains source-only receipt and usage lineage rather than allocation history', function () {
    expect(collect(MrjZiswafOpeningPosition::fundSourceHistory('SODAQOH'))->pluck('kind')->all())
        ...
        ->and(collect(MrjZiswafOpeningPosition::fundSourceHistory('DHUAFA'))->where('kind', '!=', 'account_position')->pluck('amount')->all())
...
test('historical Fund usage retains source date labels, row lineage, and separate cash composition', function () {
    ...
    $dhuafa = collect(MrjZiswafOpeningPosition::fundSourceHistory('DHUAFA'));
    ...
    expect($activity->where('kind', 'receipt')->pluck('amount')->map(fn (string $amount) => (int) $amount)->sum())->toBe(20_945_000)
        ...
        ->and($infaq->where('kind', 'account_position'))->toBeEmpty()
        ->and($dhuafa->firstWhere('kind', 'account_position'))->toMatchArray([
            'amount' => '2653000.00',
            'source_reference' => 'Sisa Alokasi Dana!D66:E66',
        ])
        ->and(MrjZiswafOpeningPosition::cashTromolOriginalComposition())->toMatchArray([
            'fund_code' => 'DHUAFA',
            'financial_account_code' => 'CASH-ZISWAF',
            'amount' => '2653000.00',
